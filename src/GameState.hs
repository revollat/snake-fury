...
opositeMovement move = case move of
  North -> South
  South -> North
  East -> West
  West -> East
...
makeRandomPoint (BoardInfo boardHeight boardWidth) randomSeed =
  (newPoint, newStdGen2)
  where
    (newX, newStdGen1) = uniformR (1, boardWidth) randomSeed
    (newY, newStdGen2) = uniformR (1, boardHeight) newStdGen1
    newPoint = (newX, newY)
...
inSnake p (SnakeSeq snakeHead snakeBody) = p == snakeHead || isJust (p `S.elemIndexL` snakeBody)
...
newApple boardInfo gs@(GameState snakeSeq applePos mov rndGen) =
  if applePos == newPoint || newPoint `inSnake` snakeSeq
    then newApple boardInfo gs {randomGen = newRndGen}
    else (newPoint, newRndGen)
  where
    (newPoint, newRndGen) = makeRandomPoint boardInfo rndGen
